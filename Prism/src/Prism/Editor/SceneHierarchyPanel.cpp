			...
			if (m_SelectionContext)
			{
				...
				if (ImGui::BeginPopup("AddComponentPanel"))
				{
					...
					if (ImGui::Button(TR("Script")))
					{
						...
						ScriptEngineManager::OnScriptAdded(m_SelectionContext, scripts.back());
						ImGui::CloseCurrentPopup();
					}
	...
	void SceneHierarchyPanel::DrawComponents(Entity entity)
	{
		...
		if (entity.HasComponent<ScriptsComponent>())
		{
			...
			if (ImGui::TreeNodeEx((void*)((uint32_t)entity | typeid(ScriptsComponent).hash_code()), ImGuiTreeNodeFlags_DefaultOpen, TR("Scripts")))
			{
					...
					if (open)
					{
						...
						if (ImGui::BeginCombo("##language", langStrings[currentLang]))
						{
							for (int lang = 0; lang < 2; lang++)
							{
								...
								{
									ScriptEngineManager::OnScriptRemoved(entity, script);
									script.Language = (ScriptLanguage)lang;
									ScriptEngineManager::OnScriptAdded(entity, script);
								}
								if (is_selected)
									ImGui::SetItemDefaultFocus();
							}
						...
						{
							uint32_t fieldCount = engine->GetFieldCount(entity.GetSceneUUID(), id, script.ModuleName);
							for (uint32_t fi = 0; fi < fieldCount; fi++)
							{
								PublicFieldInfo info;
								engine->GetFieldInfo(entity.GetSceneUUID(), id, script.ModuleName, fi, info);

								auto* field = engine->GetField(entity.GetSceneUUID(), id, script.ModuleName, info.Name);
				...
				// 寰幆澶栭儴瀹夊叏瑙﹀彂鑴氭湰绉婚櫎閫昏緫
				if (scriptToDelete != -1)
				{
					ScriptEngineManager::OnScriptRemoved(entity, sc.Scripts[scriptToDelete]);
				...
				if (ImGui::Button(TR("Add Script")))
				{
					...
					ScriptEngineManager::OnScriptAdded(entity, sc.Scripts.back());
				}
