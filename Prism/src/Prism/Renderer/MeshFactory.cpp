...
namespace Prism {
    ...
    Ref<Mesh> MeshFactory::CreateSphere(float radius)
    {
        ...
        for (float latitude = 0.0F; latitude <= latitudeBands; latitude++)
        {
            ...
            {
                float phi = longitude * 2.0F * glm::pi<float>() / longitudeBands;
        ...
        for (uint32_t lat = 0; lat < (uint32_t)latitudeBands; lat++)
        {
            for (uint32_t lon = 0; lon < (uint32_t)longitudeBands; lon++)
            {
                uint32_t first = (lat * ((uint32_t)longitudeBands + 1)) + lon;
