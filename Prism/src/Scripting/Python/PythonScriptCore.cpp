...
namespace Prism::Python {
		...
		if (m_Ref.IsValid())
		{
			...
			if (val)
			{
				...
				else if (PyLong_Check(val))
				{
					...
					memcpy(buffer, &ival, sizeof(int64_t));
				}
		...
		if (m_Ref.IsValid())
		{
			...
			if (existing)
			{
				if (PyFloat_Check(existing))
					val = PyFloat_FromDouble(*(const double*)buffer);
				else if (PyLong_Check(existing))
					val = PyLong_FromLongLong(*(const int64_t*)buffer);
	...
	void NativeModule::Register()
	{
		...
		for (auto& entry : m_Functions)
		{
			...
			data->Methods.push_back({ entry.Name.c_str(), fn, METH_VARARGS, entry.Doc.c_str() });
		}
