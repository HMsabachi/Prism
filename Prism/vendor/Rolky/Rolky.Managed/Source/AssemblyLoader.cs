...
public static class AssemblyLoader
{
	...
	private static readonly AssemblyLoadContext? s_RolkyAssemblyLoadContext;
	...
	static AssemblyLoader()
	{
		...
		s_RolkyAssemblyLoadContext = AssemblyLoadContext.GetLoadContext(typeof(AssemblyLoader).Assembly);
		s_RolkyAssemblyLoadContext!.Resolving += ResolveAssembly;

		s_AssemblyCache.Add(CORAL_ALC_CACHE_ID, new());

		CacheRolkyAssemblies();
	}

	private static void CacheRolkyAssemblies()
	{
		foreach (var assembly in s_RolkyAssemblyLoadContext!.Assemblies)
		{
	...
	internal static Assembly? ResolveAssembly(AssemblyLoadContext? InAssemblyLoadContext, AssemblyName InAssemblyName)
	{
		try
		{
			...
			if (s_RolkyAssemblyLoadContext != null)
			{
				foreach (var assembly in s_RolkyAssemblyLoadContext.Assemblies)
				{
